...
The **music is the spine** — establish one track before anything else. This skill is tuned for **fast, high-energy BGM**: a strong beat grid drives the cuts (calm tracks work, but pace by phrase rather than beat). If the user gave you audio — a music file, or a video to pull the audio from — use it. If not, source one from Freesound: choose the mood from the user's description (e.g. "driving synthwave", "trap beat", "upbeat corporate"), call `mcp__freesound-music__search_freesound_music` with that as a short concrete query and a `max_duration` matched to the video's target length, pick a CC0 result, then `mcp__freesound-music__download_freesound_music` it to `assets/bgm.mp3`. No sign-in or credential needed — Freesound's CC0 catalog is free. See `../media-use/audio/references/bgm.md`. Stage any user-supplied images or videos so frames can weave them in on the beat grid; otherwise typography carries the whole video.
...
```bash
HYPERFRAMES_SKIP_SKILLS=1 npx hyperframes init "videos/<project>" --non-interactive --example=blank
