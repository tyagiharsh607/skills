...
**Autonomous mode is NOT "skip all gates"** (brief contract § 1). It covers user-preference questions (voice, color emphasis, beat count, music yes/no, captions yes/no — where the agent decides on the user's behalf). It does NOT cover quality-verification gates. The following remain non-skippable in auto mode:
...
No sign-in or credential is needed for audio — voice comes from `tts-mcp-server` (free, no key) and BGM from `freesound-music` (free, CC0 catalog, no key).
...
If Step 2 said no narration — ask about background music, then skip to Step 5. Otherwise: pick a voice via `tts-mcp-server`, generate audio, transcribe, map timestamps to beats. Then ask about captions.
...
| [step-4-vo.md](references/step-4-vo.md)                                            | Step 4 — voice generation via tts-mcp-server, timing                                                                                           |
