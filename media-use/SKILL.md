...
## Setup — install heygen first (free-usage path, image/icon/video only)
...
This unlocks the FREE path for image/icon catalog search and avatar videos. Sign in with `--oauth` — the free allowance rides on the OAuth session (an API key bills API credits instead). `bgm`/`sfx`/`voice` need no setup at all — they're agent-mediated via the free `tts-mcp-server` and `freesound-music` MCP tools (see **Audio engine** below), no HeyGen account required. Before resolving anything image/icon/video-related, verify setup with:
...
| No voice / audio generation                | `resolve --type voice` (agent-mediated via `tts-mcp-server`, free) + the audio engine (`audio/scripts/audio.mjs`)               |
...
| ------- | -------------------------------- | ------------------------------------------------------------ |
| `bgm`   | Background music                 | Agent-mediated via `freesound-music` (CC0 catalog)            |
| `sfx`   | Sound effects                    | Agent-mediated via `freesound-music` (CC0 catalog)            |
...
| `voice` | TTS voiceover                    | Agent-mediated via `tts-mcp-server` (free, no key)            |
...
```bash
# Background music — agent-mediated: search + download via freesound-music first
# (mcp__freesound-music__search_freesound_music → download_freesound_music), THEN
# register the downloaded file:
node <SKILL_DIR>/scripts/resolve.mjs --type bgm --from ./downloaded-track.mp3 --project .
# → resolved bgm_001 → .media/audio/bgm/bgm_001.mp3 (bgm, 25s)

# Sound effect — same two-step MCP-then-register flow:
node <SKILL_DIR>/scripts/resolve.mjs --type sfx --from ./downloaded-whoosh.mp3 --project .
...
| `--provider`    | Force one generator (e.g. `codex`, `mflux`, `heygen`)                                |
...
#   [project] upbeat tech launch (25s, freesound-music.agent-mediated)
#           .media/audio/bgm/bgm_001.wav
#   [global]  energetic tech intro (22s, freesound-music.agent-mediated)
...
media-use holds no keys; every external tool owns its auth. Image/icon/video
generation is centered on the HeyGen CLI free-usage path — install and
authenticate `heygen` before resolving those types. `bgm`/`sfx`/`voice` need no
CLI and no auth at all: they're agent-mediated via the free `freesound-music`
and `tts-mcp-server` MCP tools (see **Audio engine** below) — the agent calls
the tool directly, then registers the result with `resolve --type <type>
--from <file>`. Local tools are opt-in alternatives where they exist: mflux for
image, Parakeet for transcription, and LTX for local video generation.
`resolve` spec-checks AVAILABLE RAM for those local ladders
(`describeModelLadder`); the agent can see the ladder and override.

| Type      | Provider / path                                                                  |
| --------- | -------------------------------------------------------------------------------- |
| bgm/sfx   | agent-mediated via `freesound-music` (free, CC0, no key) + `resolve --from`      |
| image     | heygen search free-usage path; optional local mflux; codex `image_gen` upsell    |
| voice     | agent-mediated via `tts-mcp-server` (free, no key) + `resolve --from`            |
...
Local mflux (image) and LTX (video) run on-device (free, private, offline once
cached). The `codex` CLI remains the ChatGPT-sub image upsell. Cost rule (X4):
the agent confirms before an agent-initiated paid call; a user-requested one
just runs.
...
leaving the project + global cache and any installed local provider. `bgm` /
`sfx` / `voice` aren't network calls from `resolve` itself (the MCP call
happens outside this script), so `--local-only` doesn't change their flow.
...
4. Search via provider (HeyGen asset search for image/icon; voice/bgm/sfx are agent-mediated — see Providers above), or resolve color locally
...
shot), use the shared engine at `audio/scripts/audio.mjs`. TTS and BGM are
produced by MCP tools the calling AGENT invokes directly (`tts-mcp-server` /
`freesound-music`) — the engine only INGESTS what's already on disk. It takes a
neutral `audio_request.json` and writes `audio_meta.json` plus assets under
`.media/audio/{voice,bgm,sfx}`:

```bash
# 1. Agent generates voice via tts-mcp-server (generate_speech/generate_batch_speech)
#    to assets/voice/<id>.mp3, and BGM via freesound-music (search + download) to
#    assets/bgm/track.mp3 — BEFORE running the engine. See audio/references/tts.md, bgm.md.
# 2. Then ingest:
node <SKILL_DIR>/audio/scripts/audio.mjs --request ./audio_request.json --out ./audio_meta.json
```

- **Request** `{ lang?, speed?, lines: [{ id, text, sfx?: [names] }], bgm: { mode?, query? } }`: `id` joins each line back to your model (and must match the filename used when generating each line's audio, `assets/voice/<id>.mp3`); `bgm.mode` = `retrieve | none` (omit for the default, `retrieve`). `--only tts,bgm,sfx` runs a subset and merges into an existing `--out`.
- **Output** `audio_meta.json` (id-keyed): `voices[].{path,duration_s,words[]}` (word timestamps for captions, via a chained Whisper pass since Edge TTS has no native timestamps), `sfx[]`, `bgm`, `total_duration_s`.
- **No sign-in, no credential, no CLI needed for audio.** Both `tts-mcp-server` and `freesound-music` are free MCP tools — generate/download the files first, then run the engine to ingest.
- BGM is retrieval-only (Freesound is a library, not a generative model) and fully synchronous — no `bgm_pending`, no `wait-bgm` step.

Transcription / background removal / captions use the `hyperframes` CLI (`transcribe`, `remove-background`), see the per-topic guides in `audio/references/` (`tts.md`, `bgm.md`, `sfx.md`, `transcribe.md`, `remove-background.md`, `captions/`).
...
free-usage path for image/icon catalog search and avatar video, so those
capabilities need `heygen` installed and authenticated. `bgm`/`sfx`/`voice`
need no CLI at all — they're agent-mediated via `freesound-music` /
`tts-mcp-server` MCP tools (see **Audio engine** above). Local tools are OPT-IN
alternatives where they exist; install one to unlock its free, private,
on-device path instead of or ahead of a cloud path for that type. Only
...
| `heygen`           | catalog (image/icon) + avatar video — the free-usage path                | `curl -fsSL https://static.heygen.ai/cli/install.sh \| bash` then `heygen auth login --oauth` (needs >= v0.3.0)     |
...
| `npx hyperframes`  | whisper.cpp (transcribe fallback), remove-background                    | bundled with the hyperframes CLI                                                                                    |
