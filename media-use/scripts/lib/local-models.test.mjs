...
test("selectModel recommends the CLI path on a CPU-only machine (Kokoro retired)", () => {
  // fish-speech needs a GPU; with Kokoro gone there's no CPU-tier tts model —
  // selectModel correctly falls through to the tts-mcp-server CLI/cloud path.
...
test("preferTier:'medium' avoids the large model even on a strong machine", () => {
  // tts no longer has a medium tier (Kokoro retired) — use upscale (real-esrgan
  // medium / seedvr2 large) to exercise the same preferTier-gating logic.
  const r = selectModel("upscale", strongGpu, { preferTier: "medium" });
...
test("selectModel gates on AVAILABLE RAM, not total, when both are present", () => {
  ...
  const r = selectModel("upscale", busy);
  assert.equal(r.tier, "medium", "available RAM (6GB) rules out the 24GB large tier");
});
