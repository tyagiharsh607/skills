...
const MODELS = {
  tts: [
    // Kokoro (the former CPU/medium-tier default) is retired: voice generation
    // is now agent-mediated via tts-mcp-server (Microsoft Edge TTS) — free, no
    // key, no local install, so there's no local CPU-tier model to recommend
    // ahead of it anymore. fish-speech remains as an advanced opt-in for users
    // who specifically want local zero-shot voice cloning on a GPU machine; on
    // a CPU-only machine selectModel("tts", …) now correctly recommends the
    // CLI/cloud (tts-mcp-server) path instead of a local model.
    {
      id: "fish-speech",
      tier: "large",
      sizeMB: 1100,
