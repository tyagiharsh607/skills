...
test("heygen provider is first for every type it still serves (image, icon)", () => {
...
test("bgm/sfx/voice are agent-mediated (freesound-music / tts-mcp-server) — heygen retired", () => {
  for (const t of ["bgm", "sfx", "voice"]) {
    const first = getProviders(t)[0];
    assert.ok(first, `no provider for ${t}`);
    assert.doesNotMatch(first.name, /^heygen|^kokoro/, `${t} still lists a retired provider`);
  }
});

test("sanctioned providers only: heygen, local mflux, codex, design spec, logo tiers, agent-mediated audio", () => {
  const allowed =
    /^heygen|^mflux\.local$|^codex\.image_gen$|^design_spec$|^svgl$|^simple-icons$|^github\.avatar$|^favicon\.ddg$|^color_grade\.local$|^cube_lut\.local$|^freesound-music\.agent-mediated$|^tts-mcp-server\.agent-mediated$/;
...
test("voice: single agent-mediated tts-mcp-server provider (Kokoro/HeyGen-TTS cascade retired)", () => {
  const ps = getProviders("voice");
  assert.equal(ps.length, 1, "kokoro/heygen-tts cascade is retired — one stub remains");
  assert.equal(ps[0].name, "tts-mcp-server.agent-mediated");
  assert.ok(!ps[0].network, "the provider function makes no network call itself");
  assert.ok(!ps[0].paid, "tts-mcp-server is free");
