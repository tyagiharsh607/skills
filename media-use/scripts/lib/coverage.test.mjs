...
// Wenbo (06-29): heygen free-usage is the default for image/icon/video; local
// models are the opt-out fallback there ("if user no, then local"). tts is a
// special case since the migration to tts-mcp-server: it's free-mediated
// (agent-mediated MCP tool) with no heygen involvement at all, and fish-speech
// is kept only as an advanced local opt-in — but the underlying claim (local
// models exist as a real, populated fallback table) still holds for tts too.
